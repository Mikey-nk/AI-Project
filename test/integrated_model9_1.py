...
class MarketDataProvider:
    ...
    def get_stock_news(self, symbol: str, count: int = 5) -> List[Dict[str, str]]:
        """Fetch recent news for a stock symbol from Yahoo Finance (internet access)"""
        try:
            ticker = yf.Ticker(symbol)
            ...
            return [
                {
                    ...
                    "link": item["link"],
                    "time": datetime.datetime.fromtimestamp(item["providerPublishTime"]).strftime("%Y-%m-%d %H:%M") if "providerPublishTime" in item else "N/A"
                }
...
class BaseAgent(ABC):
    ...
    def process_task(self, task: Task) -> str:
        ...
        # Add news context from internet
        news_context = ""
        if task.market_data:
            for symbol in task.market_data.keys():
                news = self.market_provider.get_stock_news(symbol, 3)
                if news:
                    news_summary = "\n".join([f"- {n['title']} ({n['time']})" for n in news])
...
"""
    ...
    def add_task(self, task: Task, symbols: List[str] = None):
        if not task.market_data:
            symbols_to_fetch = symbols if symbols else ['AAPL', 'MSFT', 'GOOGL', 'AMZN']
            try:
                task.market_data = self.market_provider.get_stock_data(symbols_to_fetch)
            except:
...
class ManagerAgent:
    ...
    def assign_task(self, agent_key: str, description: str, priority: TaskPriority, symbols: List[str] = None) -> str:
...
def main():
    ...
    with tab2:
        ...
        with col_left:
            ...
            symbol = st.text_input("Enter Stock Symbol:", value="AAPL", placeholder="e.g., AAPL, TSLA, any valid symbol").upper()
            ...
            if st.button("📊 Get Stock Data"):
                with st.spinner(f"Fetching data for {symbol}..."):
                    ...
                    if "error" not in stock_data:
                        ...
                        news = manager.market_provider.get_stock_news(symbol)
                        if news:
                            for article in news:
    ...
    with tab3:
        ...
        with col1:
            ...
            symbols_input = st.text_input("Symbols to analyze (comma-separated, optional):", placeholder="e.g., AAPL,MSFT,GOOGL")
            ...
            if st.button("🚀 Assign Task", type="primary", use_container_width=True):
                if task_description.strip():
                    symbols = [s.strip().upper() for s in symbols_input.split(',') if s.strip()] if symbols_input else None
                    result = manager.assign_task(selected_agent, task_description.strip(), priority, symbols=symbols)
                    st.success(f"✅ {result}")
                else:
        ...
        with col2:
            ...
            # Dynamic analyze task with variety of options
            analyze_symbol = st.selectbox("Select Symbol to Analyze:", ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'NVDA', 'META', 'NFLX'])
            if st.button("📊 Analyze Selected", key="quick_analyze", use_container_width=True):
                description = f"Provide comprehensive analysis of {analyze_symbol} stock including technical indicators and investment recommendation"
                result = manager.assign_task(
                    "quant_analyst",
                    description,
                    TaskPriority.HIGH,
                    symbols=[analyze_symbol]
                )
                st.success(f"✅ {result}")
